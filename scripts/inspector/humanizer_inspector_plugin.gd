...
func _parse_category(human, category):
	...
	scene.get_node('%SkinShaderCheckBox').button_pressed = &'skin_shader' in human.human_config.components
	...
	scene.get_node('%SkinShaderCheckBox').toggled.connect(human.set_component_state.bind(&'skin_shader'))
