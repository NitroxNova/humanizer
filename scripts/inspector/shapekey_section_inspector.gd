...
func _ready() -> void:
	...
	for key in shapekeys:
		...
		if human.humanizer.human_config != null :
			if name.begins_with('Macro') or name.begins_with('Race') or key in ['cupsize', 'firmness']:
				slider.value = human.humanizer.human_config.targets.macro[key]
			elif human.humanizer.human_config.targets.raw.has(key):
				slider.value = human.humanizer.human_config.targets.raw[key]
